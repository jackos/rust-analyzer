...
impl Default for Notebook {
    fn default() -> Self {
        Self { prefix: "fn main() {\n".to_string(), suffix: "}".to_string(),cells: Default::default(), line_offsets: Default::default(), character_offsets: Default::default() }
    }
}
...
impl Notebook {
	...
	pub fn get_program(&self) -> String {
		...
		let mut text = cells.iter().fold(self.prefix.clone(), |acc, item| {
			format!("{}{}\n", acc, item.1)
		});
	...
	pub fn get_line_offset(&self, fragment: &str) -> i32 {
		let index: u16 = fragment[2..fragment.len()].parse().unwrap();
		let sum: i32  = self.line_offsets.clone().into_iter().filter(|x| x.0 < index).map(|x| x.1 as i32).sum();
